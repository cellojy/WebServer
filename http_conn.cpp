...
void http_conn::init(int sockfd, sockaddr_in &addr)
{
    ...
    m_address = addr;
    int reuse = 1;
    setsockopt(m_sockfd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(true));
...
bool http_conn::read()
{
    ...
    while (true)
    {
        res = recv(m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SIZE - m_read_idx, 0);
        if (res == -1)
        {
...
void http_conn::process()
{
    ...
    bool write_res = process_write(res);
    if (!write_res)
    {
        close_conn();
    }
...
{
    if (m_wirte_idx >= WRITE_BUFFER_SIZE)
    {
        return false;
    }
    va_list arg_list;
    va_start(arg_list, format);
    int len = vsnprintf(m_write_buf + m_wirte_idx, WRITE_BUFFER_SIZE - m_wirte_idx - 1, format, arg_list);
    if (m_wirte_idx + len + 1 >= WRITE_BUFFER_SIZE)
        return false;
    m_wirte_idx += len;
    va_end(arg_list);
    return true;
